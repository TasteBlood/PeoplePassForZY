    ...
    private void toPreviewActivity(ArrayList<Image> images, int position) {
        if (images != null && !images.isEmpty()) {
            PreviewActivity.openActivity(this, images,
                    mAdapter.getSelectImages(), isSingle, mMaxCount, position,true);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageSelector.RESULT_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                values.put(MediaStore.Images.ImageColumns.DATA, mPhotoPath);
                values.put(MediaStore.Images.ImageColumns.TITLE,
                        mPhotoPath.substring(mPhotoPath.lastIndexOf("/") + 1));
                ...
                ArrayList<String> images = new ArrayList<>();
                if(mAdapter!=null){
                    mAdapter.getSelectImages().add(new Image(mPhotoPath,System.currentTimeMillis(),
                            mPhotoPath.substring(mPhotoPath.lastIndexOf("/") + 1),"image/jpeg"));
                    ArrayList<Image> selectImages = mAdapter.getSelectImages();
                    for (Image image : selectImages) {
                        images.add(image.getPath());
                    }
...
//            Toast.makeText(this, "没有图片", Toast.LENGTH_LONG).show();
    ...
    private File createImageFile() throws IOException {
        ...
        File tempFile = new File(storageDir+"/Camera/", imageFileName);
